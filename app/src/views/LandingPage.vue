        ...
        <div class="mx-auto max-w-6xl px-6 text-center">
          ...
          <div v-else class="relative">
            <button
              v-if="plans.length > 1"
              type="button"
              class="absolute left-0 top-1/2 hidden -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white p-3 text-slate-600 shadow-sm transition hover:border-emerald-300 hover:text-emerald-600 lg:flex"
              :class="{ 'cursor-not-allowed opacity-40': !canScrollPrev }"
            ...
            <div
              ref="pricingCarousel"
              class="flex snap-x snap-mandatory gap-6 overflow-x-auto pb-4 pr-1 scroll-smooth"
              @scroll.passive="updateCarouselNav"
            >
              <article
                v-for="plan in plans"
                :key="plan.id"
                data-plan-card
                class="min-w-[85%] snap-start rounded-2xl border border-slate-100 bg-white p-8 text-left shadow-sm transition hover:shadow-lg sm:min-w-[55%] lg:min-w-[32%]"
              >
            ...
            <button
              v-if="plans.length > 1"
              type="button"
              class="absolute right-0 top-1/2 hidden -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white p-3 text-slate-600 shadow-sm transition hover:border-emerald-300 hover:text-emerald-600 lg:flex"
              :class="{ 'cursor-not-allowed opacity-40': !canScrollNext }"
...
const pricingCarousel = ref(null);
...
function updateCarouselNav() {
  const container = pricingCarousel.value;
  ...
  const tolerance = 2;
  canScrollPrev.value = container.scrollLeft > tolerance;
  canScrollNext.value = container.scrollLeft < maxScrollLeft - tolerance;
}
...
function scrollCarousel(direction) {
  ...
  const firstCard = container.querySelector('[data-plan-card]');
...
watch(plans, () => {
  nextTick().then(updateCarouselNav);
});
