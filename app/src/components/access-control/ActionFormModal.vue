    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 px-4">
      <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <header class="flex items-center justify-between border-b border-slate-200 pb-4">
          <div>
            <h2 class="text-lg font-semibold text-slate-900">{{ isEdit ? t('accessControl.sections.actions.modal.editTitle') : t('accessControl.sections.actions.modal.createTitle') }}</h2>
            <p class="text-sm text-slate-500">{{ t('accessControl.sections.actions.modal.description') }}</p>
          </div>
        ...
        <form class="mt-4 space-y-4" @submit.prevent="submit">
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">{{ t('accessControl.sections.actions.modal.fields.name') }}</label>
            <input v-model="form.name" type="text" class="input" :placeholder="t('accessControl.sections.actions.modal.fields.placeholder')" required />
          </div>
...
const isEdit = computed(() => Boolean(props.action?.id));
