  ...
  <div class="space-y-6">
    <section class="space-y-3">
      <div class="flex flex-col gap-1">
        <label class="text-xs font-semibold uppercase tracking-wide text-slate-400">{{ t('goals.wizard.fields.template') }}</label>
        <select
          class="input"
    ...
    <section v-if="isAdmin" class="space-y-3">
      ...
      <div class="flex flex-col gap-2 md:flex-row md:items-center">
        <input
          ...
          class="input md:w-80"
          v-model="ownerQuery"
          :placeholder="t('goals.wizard.fields.ownerPlaceholder')"
        />
        <button type="button" class="btn-secondary md:w-auto" @click="triggerOwnerSearch">
          {{ t('goals.wizard.actions.searchOwner') }}
        </button>
      </div>
